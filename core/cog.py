...
class Core:
    ...
    async def install(self, ctx, *, extensions):
        ...
        async def _install(_extension):
            ...
            try:
                ...
                failed_to_install_extensions.append(_extension)
                return False
                ...
                if unsatisfied is not None:
                    ...
                    installed_extensions.append(_extension)
                    return True
    ...
    async def update(self, ctx, *, extensions):
        ...
        async def _update(extension):
            await ctx.send("Updating '**" + extension + "**'...")
            try:
                unsatisfied = self.base.update_extension(extension)
            except ExtensionNotFound:
                await ctx.send("The extension '**" + extension + "**' could not be found.")
                ...
                if unsatisfied is not None:
                    ...
                    if unsatisfied['packages']:
                        await ctx.send("Do you want to install the new requirements of " + extension + " now? (yes/no)")
                        answer = await self.bot.wait_for_answer(ctx)
                        if answer is True:
                            for package in unsatisfied['packages']:
                                return_code = self.base.install_package(package)
                    ...
                    await ctx.send("The extension '**" + extension + "**' was updated successfully.")
    ...
    async def uninstall(self, ctx, *, extensions):
        ...
        async def _uninstall(extension):
            await ctx.send("Uninstalling '**" + extension + "**'...")
            try:
                ...
                if to_cascade:
                    await ctx.send(strings.would_be_uninstalled_too.format(extension) + "\n"
                                   + "**" + "**\n**".join(to_cascade) + "**")
                    ...
                    await ctx.send("The '**" + extension + "**' extension was uninstalled successfully.")
